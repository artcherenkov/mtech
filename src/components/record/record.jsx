...
import {
  getActiveRecordId,
  getRecords,
} from "../../store/reducers/app-store/selectors";
import {
  closeRecord,
  editRecord,
  setRecordToDelete,
...
const Record = (props) => {
  ...
  return (
    <div className="popup record">
      {updatingRecord && (
        <div className="popup__content record__content-wrapper">
          <div className="record__controls">
            ...
            <button
              className="record__button record__button_type_close"
              onClick={handleCloseBtnClick}
            />
          </div>
          <div className="record__content">
            <h2 className="record__title">
              {isRecordEditing ? `Редактирование` : `Просмотр`} заявки №
              {updatingRecord.id}
            </h2>
            <RecordFields
          ...
          <div className={getTooltipStyles(isTooltipShown)}>
            ...
            <p className="record__tooltip-content">
              Удаляйте запись только в следующих случаях:
            </p>
            <ul className="record__tooltip-list">
              <li className="record__tooltip-item">в записи нет ошибок</li>
              <li className="record__tooltip-item">
                запись попала в список случайно
              </li>
              <li className="record__tooltip-item">
                расхождение является результатом внештатной ситуации, когда
                косметолог не виноват
              </li>
            </ul>
          </div>
        </div>
